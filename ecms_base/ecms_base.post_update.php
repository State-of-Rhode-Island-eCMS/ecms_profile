...
function ecms_base_post_update_041_asymmetric_paragraph_translations(&$sandbox): void {
  // Load every node and add them to the batch.
  if (empty($sandbox['nodes'])) {
    ...
    // @todo: Replace this multiple load.
    //$sandbox['nodes'] = $nodeStorage->loadMultiple();
    $sandbox['nodes'][] = $nodeStorage->load(341);
    $sandbox['total'] = count($sandbox['nodes']);
  }

  if (count($sandbox['nodes']) === 0) {
    $sandbox['#finished'] = 1;
    return;
  }

  $sandbox['#finished'] = ($sandbox['total'] - count($sandbox['nodes'])) / $sandbox['total'];

  /** @var \Drupal\node\NodeInterface $node */
  while ($node = array_shift($sandbox['nodes'])) {
    \Drupal::service('ecms_distribution.paragraph_translation_update')->updateNodeParagraphs($node);
  }

  $sandbox['#finished'] = ($sandbox['total'] - count($sandbox['nodes'])) / $sandbox['total'];
  // @todo: Load every paragraph for the node
  // @todo: Save the paragraph as a new language.
}


//function createDuplicateWithSingleLanguage(ParagraphInterface $paragraph, string $langcode): ParagraphInterface {
...
//        $item->entity = createDuplicateWithSingleLanguage($item->entity, $langcode);
//      }
//    }
//  }
//
