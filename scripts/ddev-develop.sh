...
# Symlink the profile
# https://github.com/ddev/ddev/pull/7199 DDEV needs a release for the DDEV_APPROOT variable to exist.
# Create a `custom` profile directory
ddev exec 'mkdir -p $DDEV_DOCROOT/profiles/custom/ecms_profile'

# Symlink the base profiles into the `ecms_profile` profile directory.
ddev exec 'ln -s -f $(realpath -s --relative-to=${DDEV_DOCROOT}/profiles/custom/ecms_profile ecms_base) $DDEV_DOCROOT/profiles/custom/ecms_profile'
ddev exec 'ln -s -f $(realpath -s --relative-to=${DDEV_DOCROOT}/profiles/custom/ecms_profile ecms_acquia) $DDEV_DOCROOT/profiles/custom/ecms_profile'
...
ddev exec 'test -f develop/merge.composer.json && (mv develop/merge.composer.json develop/composer.json && composer install --working-dir=develop)'

## Run composer install in the web d
