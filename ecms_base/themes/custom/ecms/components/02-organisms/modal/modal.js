/**
 * @file Content modal behavior.
 *
 * Shows the content modal (once per visitor, tracked via cookie) and wires the
 * mobile "add to home screen" helper modal.
 *
 * Bound via Drupal.behaviors + once so it also runs for modals delivered after
 * initial page load. Modals render within node/main content, which Drupal
 * streams in via BigPipe for authenticated users after DOMContentLoaded has
 * fired — a one-time binding would never show the modal.
 *
 * MicroModal is provided by the ecms/modal library.
 */
...
(function (Drupal, once) {
  ...
  Drupal.behaviors.ecmsModal = {
    attach: function (context) {
      once('ecms-modal', '.qh__modal', context).forEach(function (contentModal) {
        ...
        if (!getCookie(modalName)) {
          const homescreen = document.querySelector('.qh__modal-homescreen');
          const closeContentModal = document.querySelector('#close-modal-content');
          const closeHomescreenModal = document.querySelector('.qh__modal-homescreen__close-icon');
          ...
          if (homescreen && mobileCheck()) {
            ...
            closeHomescreenModal.addEventListener('click', function () {
